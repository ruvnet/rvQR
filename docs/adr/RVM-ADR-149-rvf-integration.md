<!--
  Mirrored from ruvnet/rvm docs/adr/ADR-149-rvf-integration.md on 2026-08-02
  — canonical source lives upstream; do not edit here.
  Renamed to `RVM-ADR-149-…` here: RVM has its own ADR numbering space, and rvQR already mirrors a different ADR-149-less RuVector set — the prefix keeps the two numbering spaces from colliding.
-->

> **Mirrored copy.** Canonical source: [`ruvnet/rvm` → `docs/adr/ADR-149-rvf-integration.md`](https://github.com/ruvnet/rvm/blob/main/docs/adr/ADR-149-rvf-integration.md), copied 2026-08-02. Edits belong upstream, not in rvQR.
>
> Renamed to `RVM-ADR-149-…` here: RVM has its own ADR numbering space, and rvQR already mirrors a different ADR-149-less RuVector set — the prefix keeps the two numbering spaces from colliding.
